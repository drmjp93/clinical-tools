document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupEventListeners() {
        searchInput.addEventListener('input', (e) => {
            ...
            debounceTimer = setTimeout(() => fetchDrugs(query), 300);
        });
    }
    ...
    // --- 4. UI RENDERING (LIST PANE) ---
    function renderLayout(drugs, query) {
        container.innerHTML = '';
        ...
        listPane.innerHTML = `<div class="mobile-hint">Tap a medicine to view details →</div>`;
        ...
        drugs.forEach((drug, index) => {
            ...
            // Dynamic Title Logic
            const { mainTitle, subTitle } = getDynamicTitles(drug, query);
            const priceStr = drug["FIRST PRICE"] ? `[INR ${drug["FIRST PRICE"]}]` : '';
            
            item.innerHTML = `
                <div class="li-name">${mainTitle}</div>
                <div class="li-brand">
                    <span>${subTitle}</span>
            ...
            item.addEventListener('click', () => {
                ...
                renderDetails(drug, detailPane, query);
        ...
        // Auto-load first item details
        if (drugs.length > 0) { 
            renderDetails(drugs[0], detailPane, query); 
        }
    }

    // --- 5. UI RENDERING (DETAIL PANE) ---
    function renderDetails(drug, pane, query) {
        const { mainTitle, isBrandPromoted } = getDynamicTitles(drug, query);
        
        // If a brand is promoted, show the molecule underneath it.
        const moleculeSubtitle = isBrandPromoted 
            ? `<div style="color:var(--text-muted); font-size: 0.9rem; margin-bottom: 12px; font-weight: 600;">Molecule: ${drug.Active_Content}</div>` 
            : '';
        ...
        pane.innerHTML = `
            ...
            <div class="detail-content">
                ...
                <div class="brands-grid">
                    ${buildBrandBox(drug["FIRST BRAND"], drug["FIRST MFG"], drug["FIRST PRICE"])}
                    ${drug["SECOND BRAND"] !== '-' ? buildBrandBox(drug["SECOND BRAND"], drug["SECOND MFG"], drug["SECOND PRICE"]) : ''}
                </div>
    ...
    // Determines if we should show Molecule or Brand as the main title
    function getDynamicTitles(drug, query) {
        const b1 = (drug["FIRST BRAND"] || "").toLowerCase();
        const b2 = (drug["SECOND BRAND"] || "").toLowerCase();
        let prominentBrand = null;

        if (b1 && b1.includes(query)) prominentBrand = drug["FIRST BRAND"];
        else if (b2 && b2.includes(query)) prominentBrand = drug["SECOND BRAND"];

        return {
            mainTitle: prominentBrand ? prominentBrand : drug.Active_Content,
            subTitle: prominentBrand ? `Molecule: ${drug.Active_Content}` : (drug["FIRST BRAND"] || 'Generic'),
            isBrandPromoted: !!prominentBrand
        };
    }

    function buildBrandBox(name, mfg, price) {
        if (!name) return '';
        const priceStr = price ? `[INR ${price}]` : '';
        return `
            <div class="brand-box">
    ...
    function getCategoryHTML(catString) {
    ...
    function getRiskClass(status, note) {
    ...
    function safetyStrip(label, status, note) {
